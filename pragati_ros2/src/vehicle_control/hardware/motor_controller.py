...
try:
    ...
    sys.path.insert(0, os.path.dirname(__file__))  # Add hardware dir for advanced_steering
    from config.constants import (
        MOTOR_IDS,
        GEAR_RATIOS,
        PHYSICAL,
...
def propagate_motor_error(func):
    """Decorator that sets degraded flag on motor command failure.

    When a decorated method raises an exception, this sets
    self._degraded[motor_id] = True (if motor_id is available)
    or self._degraded = True, logs the error, and re-raises.
    On success, clears the degraded flag for that motor_id.
    """

    @functools.wraps(func)
    def wrapper(self, *args, **kwargs):
        motor_id = kwargs.get("motor_id") or (args[0] if args else None)
        try:
            result = func(self, *args, **kwargs)
            # Clear degraded on success
            if (
                hasattr(self, "_degraded")
                and isinstance(self._degraded, dict)
                and motor_id is not None
            ):
                self._degraded[motor_id] = False
            return result
        except Exception as e:
            # Set degraded on failure
            if (
                hasattr(self, "_degraded")
                and isinstance(self._degraded, dict)
                and motor_id is not None
            ):
                self._degraded[motor_id] = True
            logger = getattr(self, "_logger", None) or getattr(self, "logger", None)
            if logger:
                logger.error(f"{func.__name__} failed (motor_id={motor_id}): {e}")
            raise
...
class VehicleMotorController:
    ...
    def __init__(self, motor_interface: MotorControllerInterface):
        ...
        # Motor groupings (use singleton instances from constants)
        self._motor_ids = MOTOR_IDS
        self._gear_ratios = GEAR_RATIOS
        self._limits = MOTOR_LIMITS
    ...
    def _set_motor_limits(self, motor_id: int, current_limit: float, velocity_limit: float):
    ...
    def emergency_stop(self):
        """Emergency stop all motors with retry on failure.

        Attempts to disable every motor up to 3 times with exponential
        backoff.  Re-raises on exhaustion — callers MUST know if the
        e-stop failed.
        """
        self._logger.critical("EMERGENCY STOP ACTIVATED")
        self._emergency_stop.set()

        backoff_times = [0.01, 0.1, 1.0]
        last_error = None
        ...
        for attempt, backoff in enumerate(backoff_times):
            try:
                ...
                # All motors disabled successfully
                return True
            except Exception as e:
                last_error = e
                self._logger.warning(
                    f"Emergency stop attempt {attempt + 1}/{len(backoff_times)} " f"failed: {e}"
                )
                if attempt < len(backoff_times) - 1:
                    time.sleep(
                        backoff
                    )  # BLOCKING_SLEEP_OK: e-stop retry backoff — safety-critical — reviewed 2026-03-14

        self._logger.error(
            f"Emergency stop FAILED after {len(backoff_times)} attempts: {last_error}"
        )
        raise RuntimeError(
            f"Emergency stop failed after {len(backoff_times)} attempts"
        ) from last_error
    ...
    def move_vehicle_distance(self, distance_mm: float, steering_angle_deg: float = 0.0) -> bool:
        """
        ...
        try:
            ...
            self._logger.info(
                "[DRIVE] move_vehicle_distance: dist=%.1f mm → %.4f rotations, steer=%.1f deg",
                distance_mm,
                rotations,
                steering_angle_deg,
            )
            ...
            for motor_id in self._motor_ids.drive_motors:
                ...
                self._logger.info(
                    "[DRIVE] motor_id=%d: current=%.4f rot → target=%.4f rot",
                    motor_id,
                    current_pos,
                    target_pos,
                )
        ...
        try:
            ...
            for motor_id in self._motor_ids.drive_motors:
                if not self._motor_interface.move_to_position(motor_id, position):
                    success = False
                    self._logger.error(
                        f"Failed to move drive motor {motor_id} to position {position}"
                    )
    ...
    def set_steering_angle(self, angle_deg: float) -> bool:
        ...
        try:
            ...
            self._logger.info(
                "[STEER] set_steering_angle: angle=%.2f deg (clamped) → %.4f rotations, motor_id=%d",
                angle_deg,
                rotations,
                self._motor_ids.STEERING_FRONT,
            )
