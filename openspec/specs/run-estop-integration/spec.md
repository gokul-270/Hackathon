# Spec: Run E-STOP Integration

## Purpose

Defines how the system handles emergency stop (E-STOP) during a scenario run, including the server-side flag mechanism, interruptible animation phases, the `estop_aborted` terminal status, and async run execution that keeps the event loop responsive.
